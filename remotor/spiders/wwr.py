...
class WwrSpider(scrapy.Spider):
    ...
    def parse_job(self, response):
        ...
        item["text"] = s.css(".listing-container").xpath("div/text()").extract()
        item["text"].extend(s.css(".listing-container").xpath("ul/li/text()").extract())
        try:
...
def test_parse_time():
    tests = [
        ("Posted Jun 20", datetime.datetime(2017, 6, 20)),
